...
try:
    ...
    from simple_filter import SimplePredictionFilter
...
def rolling_prediction(cities_df, history_df, start_year=2026, end_year=2030, prob_threshold=0.90):
    ...
    for year in range(start_year, end_year + 1):
        ...
        # 筛选：1. 概率>=阈值 2. 不与已有线路重叠 3. 拓扑冲突检测
        new_predictions = []
        overlap_count = 0
        ...
        if TOPOLOGY_AVAILABLE:
            try:
                ...
                filter_obj = SimplePredictionFilter(topology)
            except:
                pass
        ...
        for (city_a, city_b), prob in zip(candidates, probas):
            if prob >= prob_threshold:
                ...
                if filter_obj:
                    result = filter_obj.explain_filtering(city_a, city_b)
        ...
        if filter_obj:
            print(f"    过滤拓扑冲突线路: {topology_conflict_count}条")
